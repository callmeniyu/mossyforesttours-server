...
export async function createBookingFromPaymentIntent(
  ...
  try {
    ...
    if (existingBooking) {
      console.log(
        "ℹ️ Booking already exists for payment intent:",
        intent.id,
        "→",
        existingBooking._id,
      );
    ...
    if (
      ...
      console.error(
        "❌ Missing required metadata to create booking:",
        metadata,
      );
      return null;
    }
    ...
    const bookingData = {
      ...
      isVehicleBooking:
        metadata.bookingType === "single" &&
        metadata.packageType === "transfer",
    ...
    try {
      ...
      // Debug logging for private tour detection
      console.log('[WEBHOOK EMAIL] Package details:', {
        exists: !!packageDetails,
        type: packageDetails?.type,
        typeMatchPrivate: packageDetails?.type === 'private',
        typeMatchPrivateUpper: packageDetails?.type === 'Private',
        vehicle: packageDetails?.vehicle,
        vehicleLength: packageDetails?.vehicle?.length,
        seatCapacity: packageDetails?.seatCapacity,
        metadataIsVehicleBooking: metadata.isVehicleBooking,
      });

      // Robust private vehicle detection: check multiple signals
      const isPrivateByType = packageDetails && (
        packageDetails.type === 'private' ||
        packageDetails.type === 'Private'
      );
      const isPrivateByVehicle = packageDetails && (
        packageDetails.vehicle &&
        typeof packageDetails.vehicle === 'string' &&
        packageDetails.vehicle.length > 3
      );
      const isPrivateByMeta = metadata.isVehicleBooking === 'true';

      const isPrivateVehicleBooking = Boolean(
        isPrivateByType || isPrivateByVehicle || isPrivateByMeta
      );

      console.log('[WEBHOOK EMAIL] Private vehicle detection:', {
        isPrivateByType,
        isPrivateByVehicle,
        isPrivateByMeta,
        isPrivateVehicleBooking,
      });
      ...
      const emailData = {
        ...
        packageName:
          packageDetails?.title || `${bookingData.packageType} Package`,
        ...
        pickupGuidelines:
          packageDetails?.details?.pickupGuidelines ||
          packageDetails?.details?.pickupDescription,
        total: bookingData.total,
        currency: bookingData.paymentInfo.currency,
        isVehicleBooking: isPrivateVehicleBooking,
        vehicleName: isPrivateVehicleBooking
          ? packageDetails?.vehicle
          : undefined,
        vehicleSeatCapacity: isPrivateVehicleBooking
          ? packageDetails?.seatCapacity
          : undefined,
      };

      console.log('[WEBHOOK EMAIL] Final email data:', {
        isVehicleBooking: emailData.isVehicleBooking,
        vehicleName: emailData.vehicleName,
        vehicleSeatCapacity: emailData.vehicleSeatCapacity,
      ...
      if (emailSent) {
        ...
        console.error(
          "⚠️ Failed to send confirmation email for webhook-created booking",
        );
      }
    } catch (emailError) {
      console.error(
        "❌ Error sending confirmation email from webhook:",
        emailError,
      );
...
export async function stripeWebhook(req: Request, res: Response) {
  ...
  try {
    ...
    if (!Buffer.isBuffer(rawBody)) {
      ...
      throw new Error(
        "Request body must be a raw Buffer for signature verification",
      );
    }
  ...
  try {
    ...
    switch (event.type) {
      ...
      case "payment_intent.succeeded": {
        ...
        if (bookingId) {
          existingBooking =
            await require("../models/Booking").default.findById(bookingId);
        }

        if (!existingBooking) {
          // No existing booking — create it from payment intent metadata (primary flow)
          console.log(
            "📝 WEBHOOK: Creating booking from payment intent metadata...",
          );
          existingBooking = await createBookingFromPaymentIntent(intent);

          if (!existingBooking) {
            console.error(
              "🚨 WEBHOOK: Failed to create booking from payment intent:",
              intent.id,
            );
            try {
              await (FailedWebhookEvent as any).create({
                ...
                errorMessage:
                  "Failed to create booking from payment intent metadata",
                resolved: false,
              });
            } catch (logError) {
              console.error(
                "❌ Failed to log failed booking creation:",
                logError,
              );
            }
          } else {
            console.log(
              "✅ WEBHOOK: Booking created successfully:",
              existingBooking._id,
            );
          }
        } else {
          ...
          console.log(
            "✅ WEBHOOK: Booking already exists, confirming:",
            existingBooking._id,
          );
      ...
      case "payment_intent.payment_failed": {
        ...
        await BookingService.handleStripeFailure({
          bookingId,
          paymentIntentId: intent.id,
          reason,
        });
      ...
      case "payment_intent.canceled": {
        ...
        const reason =
          ...
          "Payment cancelled or abandoned";

        console.log(
          "🔍 Processing payment_intent.canceled for booking:",
          bookingId,
          "| Reason:",
          reason,
        );
